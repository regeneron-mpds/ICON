...
#' filter out doublets and dead cells based on scRNA data
#'
#' @param rna a Seurat RNA object
#' @param adt a Seurat ADT object
#' @param percent_mito threshold of the ratio of mitochondria gene expression to all gene expression
#' @param num_feature_min threshold of minimum number of gene expressed
#' @param num_feature_max threshold of maximum number of gene expressed
#'
#' @return a dextramer UMI matrix of QC passed cells
#' @export
selectGoodCells <- function(rna, adt, percent_mito = 0.2, num_feature_min = 200, num_feature_max = 2500){
  if (missing(rna))
    stop("Need to input a matrix or data frame")
  if (missing(adt))
    stop("Need to input a matrix or data frame")
  if(is.numeric(percent_mito) == FALSE) stop("'percent_mito' must be numeric")
  if(is.numeric(num_feature_min) == FALSE) stop("'num_feature_min' must be numeric")
  if(is.numeric(num_feature_max) == FALSE) stop("'num_feature_max' must be numeric")

  rna <- rna[which(rna$percent.mito < percent_mito & rna$nFeature_RNA > num_feature_min & rna$nFeature_RNA < num_feature_max),]
  adt<-adt[,colnames(adt) %in% rownames(rna)]
  return(as.matrix(adt))
}
